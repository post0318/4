 ...
 *
 * 모든 날짜는 UTC 자정 기준으로 다룬다(프로젝트 전역 규칙 — ntnfPricing 참고).
 */
...
export function brazilHolidayList(year: number): BrazilHoliday[] {
  ...
  const list: BrazilHoliday[] = [
    { date: new Date(Date.UTC(year, 0, 1)), name: "신정" },
    { date: addDays(easter, -48), name: "카니발" },
    { date: addDays(easter, -47), name: "카니발" },
    { date: addDays(easter, -2), name: "성금요일" },
    { date: new Date(Date.UTC(year, 3, 21)), name: "치라덴치스의 날" },
    { date: new Date(Date.UTC(year, 4, 1)), name: "노동절" },
    { date: addDays(easter, 60), name: "성체축일" },
    { date: new Date(Date.UTC(year, 8, 7)), name: "독립기념일" },
    { date: new Date(Date.UTC(year, 9, 12)), name: "아파레시다 성모의 날" },
    { date: new Date(Date.UTC(year, 10, 2)), name: "위령의 날" },
    { date: new Date(Date.UTC(year, 10, 15)), name: "공화국 선포일" },
    { date: new Date(Date.UTC(year, 11, 25)), name: "크리스마스" },
  ];
  if (year >= 2024) {
    list.push({ date: new Date(Date.UTC(year, 10, 20)), name: "흑인 의식의 날" });
  }
  return list;
}
...
export function brazilBusinessDaysBetween(start: Date, end: Date): number {
  ...
  // 수백만 회 돌며 탭을 멈추는 것을 막는다. NaN은 상위 계산(PU·듀레이션 등)으로
  // 전파돼 값이 "-"로 표시되고 주문·수량이 생성되지 않는다(멈춤만 확실히 해소).
